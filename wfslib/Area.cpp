...
std::shared_ptr<DataBlock> Area::GetDataBlock(uint32_t block_number, Block::BlockSize size, uint32_t data_size, const DataBlock::DataBlockHash& data_hash, bool check_hash) {
	return DataBlock::LoadBlock(this->device, header_block->GetBlockNumber() + ToBasicBlockNumber(block_number), size, data_size, IV(block_number), data_hash, check_hash);
}
