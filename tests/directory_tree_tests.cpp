...
TEST_CASE("DirectoryTreeTests") {
  ...
  SECTION("insert and erase entries randomlly") {
    ...
    auto one_char = std::ranges::to<std::vector>(
        std::views::cartesian_product(chars) | std::views::transform([](auto tuple) { return tuple_to_string(tuple); }));
    auto two_chars =
    ...
    auto three_chars = std::ranges::to<std::vector>(
        std::views::cartesian_product(chars, chars, chars) |
        std::views::transform([](auto tuple) { return tuple_to_string(tuple); }));
    auto four_chars = std::ranges::to<std::vector>(
        std::views::cartesian_product(chars, chars, chars, chars) |
        std::views::transform([](auto tuple) { return tuple_to_string(tuple); }));
    auto keys = std::ranges::to<std::vector>(
        std::views::join(std::vector{one_char, two_chars, three_chars, four_chars}));
