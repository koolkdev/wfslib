...
TEST_CASE_METHOD(FTreesFixture, "FTrees find returns exact and nearest extents", "[ftrees][unit]") {
  ...
  REQUIRE(CollectFreeExtents(std::ranges::subrange(it, ftrees.end())) == ExpectedEvenKeyExtents(522 / 2, kFTreesItems));

  it = ftrees.find(840);
  REQUIRE((*it).key() == 840);
  REQUIRE(CollectFreeExtents(std::ranges::subrange(ftrees.begin(), it)) == ExpectedEvenKeyExtents(0, 840 / 2));
  REQUIRE(CollectFreeExtents(std::ranges::subrange(it, ftrees.end())) == ExpectedEvenKeyExtents(840 / 2, kFTreesItems));
