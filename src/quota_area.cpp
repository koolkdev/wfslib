...
std::expected<std::vector<QuotaArea::QuotaFragment>, WfsError> QuotaArea::AllocAreaBlocks(uint32_t blocks_count) {
  ...
  if (!res)
    return std::unexpected(kNoSpace);
  auto fragment_view = *res | std::views::transform(
                                  [](const auto& frag) { return QuotaFragment{frag.block_number, frag.blocks_count}; });
  return std::ranges::to<std::vector>(fragment_view);
}
...
void QuotaArea::Init(std::shared_ptr<Area> parent_area,
  ...
  auto quota_free_blocks = std::ranges::to<std::vector>(quota_free_blocks_view);
