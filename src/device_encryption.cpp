...
void DeviceEncryption::CalculateHash(const std::span<uint8_t>& data, const std::span<uint8_t>& hash) {
  bool hash_in_block =
      data.size() >= hash.size() && &data[0] <= &hash[0] && &hash[0] + hash.size() <= &data[0] + data.size();
...
bool DeviceEncryption::CheckHash(const std::span<uint8_t>& data, const std::span<uint8_t>& hash) {
