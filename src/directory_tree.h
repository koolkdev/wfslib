 ...
 private:
  ...
  void merge_copy(DirectoryTree& new_tree,
    ...
    if (merge_node == node) {
      ...
      std::string new_merge_prefix(node.prefix());
    ...
    auto leaf = node.leaf();
    std::string merge_prefix_storage;
    std::string_view prefix_to_use = node.prefix();
    if (!merge_prefix.empty()) {
      merge_prefix_storage = std::string(merge_prefix);
      merge_prefix_storage.append(node.prefix());
      prefix_to_use = merge_prefix_storage;
    }
    auto new_node = *new_tree.alloc_new_node(prefix_to_use, node, leaf);
    if (leaf) {
      copy_value(new_tree, new_node, *leaf);
      new_tree.mutable_extra_header()->records_count += 1;
    }
